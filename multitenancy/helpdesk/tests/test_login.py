...
class TestLoginRedirect(TestCase):
    ...
    def test_custom_login_view_with_url(self):
        ...
        response = self.client.get(reverse('multitenancy.helpdesk:login'))
        # We expect that that multitenancy.helpdesk:home url is passed as next parameter in
        # the redirect url, so that the custom login can redirect the browser
        # back to helpdesk after the login.
        home_url = reverse('multitenancy.helpdesk:home')
    ...
    def test_custom_login_next_param(self):
        ...
        url = reverse('multitenancy.helpdesk:login') + "?next=" + next_param
        response = self.client.get(url)
        expected = '/custom/login/?next={}'.format(next_param)
        self.assertRedirects(response, expected, fetch_redirect_response=False)

    @override_settings(LOGIN_URL='multitenancy.helpdesk:login', SITE_ID=1)
    def test_default_login_view(self):
        """Test that default login is used when LOGIN_URL is multitenancy.helpdesk:login"""
        response = self.client.get(reverse('multitenancy.helpdesk:login'))

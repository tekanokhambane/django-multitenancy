...
class Invoice(models.Model):
    ...
    credit_used = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.0'))
...
class Refund(models.Model):
    ...
    def __str__(self) -> str:
        return f"Invoice -{self.invoice.pk}, Amount: {self.amount}, Status: {self.status}"

    def clean(self):
        if self.amount > self.invoice.amount:
            raise ValidationError("Refund amount cannot be greater than invoice amount.")
        super().clean()
...
class Credit(models.Model):
            customer = models.ForeignKey(get_user_model(), on_delete=models.CASCADE, related_name='credits')
...
class PaymentGateWay(models.Model):
    name = models.CharField(max_length=250, unique=True, help_text="The name of the payment gateway.")
    description = models.TextField(help_text="A description of the payment gateway.")
    status = models.CharField(max_length=10, choices=[("active", "Active"), ("inactive", "Inactive")], help_text="The status of the payment gateway.")
    default = models.BooleanField(default=False, help_text="Whether this payment gateway is the default gateway.")
...
class Payment(models.Model):
    ...
    created_at = models.DateTimeField(
        auto_now_add=True,
        help_text="The date and time the payment was created."
    )
    updated_at = models.DateTimeField(
        auto_now=True,
        help_text="The date and time the payment was last updated."
    )
    ...
    failed_reason = models.TextField(
        blank=True,
        null=True,
        help_text="The reason for a failed payment."
    )
...
class PaymentHandler:
    class PaymentHandler:
        ...
        def process_payment(self, payment, amount):
            ...
            self.logger.info(f"Payment processed successfully for payment {payment.id} with amount {amount}")
            return result
        ...
        def refund_payment(self, payment, amount):
            ...
            try:
                ...
                result = {'success': False, 'message': str(e)}
            self.logger.info(f"Refund processed successfully for payment {payment.id} with amount {amount}")
            return result

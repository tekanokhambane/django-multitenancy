...
from multitenancy.subscriptions.views import DeletePlanView, FeatureCreateView, PlanDetailView, PlanListView, PlanView, PlanViewSet, ProductTypeViewSet, SubscriptionsListView, UpdatePlanView
...
class TestPlanListView(unittest.TestCase):
    ...
    def test_filter_plans_by_id(self):
        ...
        plan1 = Plan.objects.create(name='Plan 1', slug='plan-1', description='Plan 1 description', price=100)
        plan2 = Plan.objects.create(name='Plan 2', slug='plan-2', description='Plan 2 description', price=200)
        plan3 = Plan.objects.create(name='Plan 3', slug='plan-3', description='Plan 3 description', price=300)
    ...
    def test_plan_list_view_filter_by_slug(self):
        ...
        plan = Plan.objects.create(name='Test Plan', slug='test-plan', description='Test Description', price=100)
...
class TestCreatePlanView(unittest.TestCase):
    ...
    def test_create_plan_successfully(self):
        ...
        response = self.client.post(reverse_lazy('create_plan'), data={'name': 'Test Plan', 'description': 'Test Description', 'price': 100})
    ...
    def test_create_plan_with_minimum_fields(self):
        # Create a user with admin type
        admin_user = User.objects.create_user(username='admin', password='admin', type='Admin')
    
        # Log in the admin user
        self.client.login(username='admin', password='admin')
    
        # Send a POST request to the CreatePlanView with the minimum required fields
        response = self.client.post(reverse_lazy('create_plan'), data={'name': 'Basic Plan', 'price': 50})
    ...
    def test_create_plan_successfully(self):
        ...
        admin_user = User.objects.create_user(username='admin', password='admin123', type='Admin')
    
        # Log in the admin user
        self.client.login(username='admin', password='admin123')
    
        # Send a POST request to the CreatePlanView with all fields filled
        response = self.client.post(reverse_lazy('create_plan'), {
            'name': 'Test Plan',
    ...
    def test_duplicate_name_not_created(self):
        ...
        unique_plan = Plan(name='Unique Plan', description='Unique description', price=100)
        unique_plan.save()
    
        # Create a form with the same name as the unique plan
        form_data = {'name': 'Unique Plan', 'description': 'Duplicate description', 'price': 200}
    ...
    def test_invalid_price_error_message(self):
        ...
        response = self.client.post(reverse_lazy('create_plan'), data={'name': 'Basic Plan', 'description': 'Basic plan description', 'price': 'abc'})
    ...
    def test_invalid_name_not_created(self):
        ...
        request = self.client.post(reverse_lazy('create_plan'), data={'name': 'Invalid Name'})
    ...
    def test_invalid_description_error_message(self):
        ...
        response = self.client.post(reverse('create_plan'), data={'name': 'Basic Plan', 'description': '', 'price': 50})
    ...
    def test_create_plan_with_features(self):
        ...
        response = self.client.post(reverse_lazy('create_plan'), data={
            'name': 'Plan 1',
...
class TestPlanDetailView(unittest.TestCase):
    ...
    def test_plan_features_displayed_correctly(self):
        # Create a plan
        plan = Plan.objects.create(name="Basic Plan", slug="basic-plan", description="Basic plan description", price=50)
    ...
    def test_plan_price_displayed_correctly(self):
        # Create a plan
        plan = Plan.objects.create(name="Basic Plan", slug="basic-plan", description="Basic plan description", price=100)
    ...
    def test_no_features_displayed_correctly(self):
        ...
        plan = Plan.objects.create(name="Basic Plan", slug="basic-plan", description="This is a basic plan", price=50)
    ...
    def test_save_plan_with_new_feature(self):
        # Create a new Plan
        plan = Plan(name="Test Plan", slug="test-plan", description="Test Description", price=100)
        plan.save()
    ...
    def test_save_plan_with_new_name(self):
        ...
        plan = Plan(name="New Plan", slug="new-plan", description="New plan description", price=100)
...
class TestFeatureCreateView(unittest.TestCase):
    ...
    def test_valid_form_feature_created_and_added_to_plan(self):
        ...
        with patch("django.shortcuts.render") as mock_render, \
             patch("sweetify.success") as mock_success, \
             patch("django.http.HttpResponseRedirect") as mock_redirect, \
             patch("multitenancy.subscriptions.views.ProductFeatureForm", return_value=form), \
             patch("multitenancy.subscriptions.views.ProductFeature.objects.create", return_value=feature), \
             patch("multitenancy.subscriptions.views.Plan.objects.get", return_value=plan):
            ...
            ProductFeature.objects.create.assert_called_once_with(name="Test Feature", description="Test Description")
            plan.features.add.assert_called_once_with(feature)
            sweetify.success.assert_called_once_with(request, "Successfully Added Feature!", icon='success', timer=5000)
    ...
    def test_plan_not_exist_redirect(self):
        ...
        render.assert_called_with(request, 'multitenancy/subscriptions/plan_detail.html', {'feature_form': form})
    ...
    def test_plan_name_and_id_mismatch(self):
        # Create a mock request object
        request = self.client.post('/admin/feature/create/', {
            'name': 'Test Feature',
            'description': 'Test Description',
            'plan': 1,
...
class TestUpdatePlanView(unittest.TestCase):
    def setUp(self):
        self.factory = RequestFactory()
        self.client = Client()
    # Test that the plan is updated successfully when the user is authenticated as an admin and the form is valid
    def test_update_plan_successfully(self):
        ...
        # Create a plan
        plan = Plan.objects.create(name='Basic', slug='basic', description='Basic plan', price=50)
    
        # Send a POST request to update the plan
        response = self.client.post(reverse_lazy('update_plan', kwargs={'pk': plan.pk}), data={'name': 'Premium', 'description': 'Premium plan', 'price': 100})
        ...
        self.assertRedirects(response, reverse_lazy('plan_list', urlconf="multitenancy.urls"))
    ...
    def test_update_plan_with_new_features(self):
        ...
        # Submit the form
        response = self.client.post(reverse_lazy('update_plan', kwargs={'pk': plan.pk}), data=form_data)
    ...
    # Test that the UpdatePlanView class successfully updates a plan with a new price when the user is authenticated as an admin and the form is valid.
    def test_update_plan_successfully(self):
        # Create a user with admin privileges
        admin_user = User.objects.create(username='admin', type='Admin')
        admin_user.set_password('admin')
        ...
        # Send a POST request to update the plan with a new price
        response = self.client.post(reverse_lazy('update_plan', kwargs={'pk': plan.pk}), {'name': 'Basic', 'slug': 'basic', 'description': 'Basic plan', 'price': 75})
    
        # Check that the plan was updated successfully
        self.assertEqual(response.status_code, 302)
        updated_plan = Plan.objects.get(pk=plan.pk)
        self.assertEqual(updated_plan.price, 75)


    # Test that the plan is updated successfully with a new name when the user is authenticated as an admin and the form is valid
    def test_update_plan_successfully(self):
        # Create a user with admin privileges
        admin_user = User.objects.create(username='admin', type='Admin')
        admin_user.set_password('admin123')
        ...
        # Create a plan
        plan = Plan.objects.create(name='Plan 1', slug='plan-1', description='Description 1', price=100)
    
        # Send a POST request to update the plan with a new name
        response = self.client.post(reverse_lazy('update_plan', kwargs={'pk': plan.pk}), {'name': 'New Plan'})
    
        # Check that the plan is updated successfully
        self.assertEqual(response.status_code, 302)
        updated_plan = Plan.objects.get(pk=plan.pk)
        self.assertEqual(updated_plan.name, 'New Plan')
    ...
    def test_plan_name_already_exists(self):
        ...
        existing_plan = Plan.objects.create(name='Existing Plan', slug='existing-plan', price=100)
    
        # Create a request object with the necessary data
        request = self.client.post(reverse_lazy('update_plan', kwargs={'pk': existing_plan.pk}), data={'name': 'Existing Plan'})
    
        # Check that the form returns errors
        self.assertFormError(request, 'form', 'name', 'Plan with this Name already exists.')
    ...
    def test_slug_not_unique(self):
        ...
        plan = Plan.objects.create(name='Plan 1', slug='plan-1', description='Description', price=100)
    
        # Create a form with the same slug
        form_data = {'name': 'Plan 2', 'slug': 'plan-1', 'description': 'Description', 'price': 200}
        form = PlanForm(data=form_data)
    ...
    def test_delete_plan_successfully(self):
        ...
        response = self.client.delete(reverse_lazy('update_plan', kwargs={'pk': plan.pk}), urlconf="multitenancy.urls")
        ...
        self.assertEqual(response.url, reverse_lazy('plan_list', urlconf="multitenancy.urls"))
...
class TestDeletePlanView(unittest.TestCase):
    ...
    def test_delete_plan_successfully(self):
        ...
        # Create a plan
        plan = Plan.objects.create(name='Plan 1', slug='plan-1', description='Plan 1 description', price=100)
    
        # Send a delete request for the plan
        response = self.client.post(reverse_lazy('delete_plan', kwargs={'pk': plan.pk}), follow=True)
    ...
    def test_delete_nonexistent_plan_as_admin(self):
        ...
        response = self.client.post(reverse_lazy('delete_plan', kwargs={'pk': 999}), follow=True)
    ...
    def test_delete_plan_with_invalid_data(self):
        ...
        plan = Plan.objects.create(name='', slug='invalid-plan', description='Invalid plan')
    ...
    def test_delete_plan_with_associated_data(self):
        ...
        plan = Plan.objects.create(name='Plan 1', slug='plan-1', description='Plan 1 description')
        feature = ProductFeature.objects.create(name='Feature 1')
        plan.features.add(feature)
        plan.save()
        ...
        self.assertContains(response, 'This plan cannot be deleted because it has associated data.')
    ...
    def test_cancel_deletion_as_admin(self):
        ...
        response = self.client.post(reverse_lazy("delete_plan", kwargs={"pk": plan.pk}), data={"cancel": True})
    ...
    def test_delete_plan_while_another_admin_editing(self):
        ...
        # Create a plan
        plan = Plan.objects.create(name='Test Plan', slug='test-plan', description='Test Description')
    ...
    def test_delete_plan_while_another_admin_deleting(self):
        ...
        self.assertContains(response, 'Another admin is currently deleting this plan. Please try again later.')
...
class TestSubscriptionsListView(unittest.TestCase):
    ...
    def test_render_subscriptions_list_template(self):
        ...
        self.assertTemplateUsed(response, 'multitenancy/subscriptions/subscriptions_list.html')
    ...
    def test_sort_subscriptions_by_field(self):
        # Create test data
        subscription1 = Subscription.objects.create(cycle='monthly', subscription_duration=30, start_date=datetime.date(2022, 1, 1), created_date=datetime.date(2022, 1, 1), end_date=datetime.date(2022, 1, 31), renewal_date=datetime.date(2022, 1, 31), reference='Test Reference 1', status='active')
        subscription2 = Subscription.objects.create(cycle='monthly', subscription_duration=30, start_date=datetime.date(2022, 2, 1), created_date=datetime.date(2022, 2, 1), end_date=datetime.date(2022, 2, 28), renewal_date=datetime.date(2022, 2, 28), reference='Test Reference 2', status='active')
        subscription3 = Subscription.objects.create(cycle='monthly', subscription_duration=30, start_date=datetime.date(2022, 3, 1), created_date=datetime.date(2022, 3, 1), end_date=datetime.date(2022, 3, 31), renewal_date=datetime.date(2022, 3, 31), reference='Test Reference 3', status='active')
        ...
        self.assertEqual(response1.context_data['object_list'], [subscription3, subscription2, subscription1])
        self.assertEqual(response2.context_data['object_list'], [subscription1, subscription2, subscription3])
        self.assertEqual(response3.context_data['object_list'], [subscription1, subscription2, subscription3])
...
class TestPlanViewSet(unittest.TestCase):
    ...
    def test_create_plan_with_existing_name(self):
        ...
        self.assertEqual(serializer.errors["name"][0], "Plan with this name already exists.")
    ...
    def test_create_plan_with_invalid_price(self):
        ...
        self.assertFalse(serializer.is_valid())
        self.assertEqual(serializer.errors["price"][0], "Ensure this value is greater than or equal to 0.")


    # Test that updating a plan with a name that already exists raises an error
    def test_update_plan_with_existing_name(self):
        # Create a plan with a unique name
        plan1 = Plan.objects.create(name="Plan 1", slug="plan-1", description="Description 1", price=100)
    
        # Create another plan with a unique name
        plan2 = Plan.objects.create(name="Plan 2", slug="plan-2", description="Description 2", price=200)
    ...
    def test_perform_create_sets_owner_correctly(self):
        ...
        request = RequestFactory().post('/plans/', {'name': 'Test Plan', 'slug': 'test-plan', 'price': 100})
    ...
    def test_serializer_returns_expected_values_for_features_field(self):
        ...
        expected_features = [{"id": feature1.id, "name": "Feature 1"}, {"id": feature2.id, "name": "Feature 2"}]
...
class TestProductTypeListView(unittest.TestCase):
    ...
    def test_view_returns_all_product_types_for_authenticated_admin_user(self):
        ...
        self.assertQuerysetEqual(response.context['object_list'], product_types, transform=lambda x: x)
...
class TestSubscriptionsViewSet(unittest.TestCase):
    ...
    def test_retrieve_all_subscriptions(self):
        ...
        subscription1 = Subscription.objects.create(cycle='monthly', subscription_duration=30, start_date=datetime.date.today(), end_date=datetime.date.today(), created_date=datetime.date.today(), renewal_date=datetime.date.today(), reference='Reference 1', last_updated=datetime.datetime.now(), product_type=ProductType.objects.create(name='Product Type 1'), reason='Reason 1', status='active')
        subscription2 = Subscription.objects.create(cycle='weekly', subscription_duration=7, start_date=datetime.date.today(), end_date=datetime.date.today(), created_date=datetime.date.today(), renewal_date=datetime.date.today(), reference='Reference 2', last_updated=datetime.datetime.now(), product_type=ProductType.objects.create(name='Product Type 2'), reason='Reason 2', status='inactive')
        ...
        self.assertEqual(response.data[0]['subscription_duration'], subscription1.subscription_duration)
        self.assertEqual(response.data[0]['start_date'], subscription1.start_date.strftime('%Y-%m-%d'))
        self.assertEqual(response.data[0]['end_date'], subscription1.end_date.strftime('%Y-%m-%d'))
        self.assertEqual(response.data[0]['created_date'], subscription1.created_date.strftime('%Y-%m-%d'))
        self.assertEqual(response.data[0]['renewal_date'], subscription1.renewal_date.strftime('%Y-%m-%d'))
        self.assertEqual(response.data[0]['reference'], subscription1.reference)
        self.assertEqual(response.data[0]['last_updated'], subscription1.last_updated.strftime('%Y-%m-%dT%H:%M:%S.%fZ'))
        self.assertEqual(response.data[0]['product_type']['name'], subscription1.product_type.name)
        ...
        self.assertEqual(response.data[1]['subscription_duration'], subscription2.subscription_duration)
        self.assertEqual(response.data[1]['start_date'], subscription2.start_date.strftime('%Y-%m-%d'))
        self.assertEqual(response.data[1]['end_date'], subscription2.end_date.strftime('%Y-%m-%d'))
        self.assertEqual(response.data[1]['created_date'], subscription2.created_date.strftime('%Y-%m-%d'))
        self.assertEqual(response.data[1]['renewal_date'], subscription2.renewal_date.strftime('%Y-%m-%d'))
        self.assertEqual(response.data[1]['reference'], subscription2.reference)
        self.assertEqual(response.data[1]['last_updated'], subscription2.last_updated.strftime('%Y-%m-%dT%H:%M:%S.%fZ'))
        self.assertEqual(response.data[1]['product_type']['name'], subscription2.product_type.name)
    ...
    def test_retrieve_subscription_by_id(self):
        ...
        subscription = Subscription.objects.create(cycle="monthly", subscription_duration=30, start_date=datetime.date.today(), end_date=datetime.date.today(), created_date=datetime.date.today(), renewal_date=datetime.date.today(), reference="Test Subscription", last_updated=datetime.datetime.now(), product_type=None, reason="Test Reason", status="active")
        ...
        self.assertEqual(response.data["subscription_duration"], subscription.subscription_duration)
        self.assertEqual(response.data["start_date"], subscription.start_date.isoformat())
        self.assertEqual(response.data["end_date"], subscription.end_date.isoformat())
        self.assertEqual(response.data["created_date"], subscription.created_date.isoformat())
        self.assertEqual(response.data["renewal_date"], subscription.renewal_date.isoformat())
        self.assertEqual(response.data["reference"], subscription.reference)
        self.assertEqual(response.data["last_updated"], subscription.last_updated.isoformat())
    ...
    def test_update_existing_subscription(self):
        # Create a subscription object
        subscription = Subscription.objects.create(cycle="monthly", subscription_duration=30, start_date=datetime.date.today(), end_date=datetime.date.today() + datetime.timedelta(days=30), created_date=datetime.date.today(), renewal_date=datetime.date.today(), reference="Test Subscription", last_updated=datetime.datetime.now(), product_type=ProductType.objects.create(name="Test Product Type"), reason="Test Reason", status="active")
        ...
        self.assertEqual(updated_subscription.end_date, datetime.date.today() + datetime.timedelta(days=7))
    ...
    def test_delete_subscription(self):
        # Create a subscription object
        subscription = Subscription.objects.create(cycle="monthly", subscription_duration=30, start_date=datetime.date.today(), end_date=datetime.date.today(), created_date=datetime.date.today(), renewal_date=datetime.date.today(), reference="Test Reference", last_updated=datetime.datetime.now(), product_type=None, reason="Test Reason", status="active")
...
class TestPlanView(unittest.TestCase):
    ...
    def test_create_new_tenant_with_valid_plan_and_subscription_duration(self):
        ...
        with patch('json.loads', return_value={'subscription_duration': 30, 'plan': 1}), \
             patch('Plan.objects.get', return_value=plan), \
             patch('request.user', return_value=user), \
             patch('uuid.uuid1', return_value='mock_uuid'), \
             patch('get_tenant_domain_model', return_value=domain_model), \
             patch('DomainModel.objects.filter', return_value=Mock(exists=Mock(return_value=False)))), \
             patch('Subscription.objects.create', return_value=subscription), \
             patch('Tenant.objects.create', return_value=tenant), \
             patch('get_tenant_domain_model().objects.create', return_value=Mock()), \
             patch('Tenant.add_user'), \
             patch('sweetify.success'), \
             patch('JsonResponse') as mock_json_response:
        ...
        sweetify.success.assert_called_with(request, "Successfully Created Tenant!", icon='success', timer=5000)
    ...
    def test_return_json_response(self):
        ...
        with patch("json.loads", return_value={"plan": 1, "subscription_duration": 30}), \
             patch("PlanView.Plan.objects.get", return_value=plan), \
             patch("PlanView.Subscription.objects.create", return_value=subscription), \
             patch("PlanView.Tenant.objects.create", return_value=tenant), \
             patch("PlanView.get_tenant_domain_model", return_value=domain_model), \
             patch("PlanView.JsonResponse", return_value=json_response) as mock_json_response:
            ...
            mock_json_response.assert_called_with({"plan": 1, "subscription_duration": 30})

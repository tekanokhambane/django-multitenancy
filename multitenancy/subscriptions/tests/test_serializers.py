...
class PlanSerializerTestCase(TestCase):
    ...
    def setUp(self):
        self.product_feature_1 = ProductFeature.objects.create(name="Feature 1")
        self.product_feature_2 = ProductFeature.objects.create(name="Feature 2")
...
class ProductFeatureSerializerTestCase(TestCase):
    ...
    def setUp(self):
        self.product_feature = ProductFeature.objects.create(name="Feature 1")
...
class SubscriptionSerializerTestCase(TestCase):
    ...
    def test_subscription_serializer(self):
        ...
        self.assertEqual(
            self.subscription.subscription_duration,
            deserialized_data["subscription_duration"],
        )
        self.assertEqual(
            start_date.strftime("%Y-%m-%d"), deserialized_data["start_date"]
        )
        self.assertEqual(end_date.strftime("%Y-%m-%d"), deserialized_data["end_date"])
        self.assertEqual(
            self.subscription.created_date.strftime("%Y-%m-%d"),
            deserialized_data["created_date"],
        )
        self.assertEqual(
            self.subscription.renewal_date.strftime("%Y-%m-%d"),
            deserialized_data["renewal_date"],
        )

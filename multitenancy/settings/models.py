...
class GeneralInfo(BaseSetting):
    ...
    company_name = models.CharField(
        max_length=255, blank=True, default="Django Multitenancy"
    )
...
class Address(BaseSetting):
    ...
    address_line_1 = models.CharField(
        max_length=200, blank=True, help_text="Enter the first line of the address"
    )
    address_line_2 = models.CharField(
        max_length=200,
        blank=True,
        null=True,
        help_text="Enter the second line of the address",
    )
    ...
    postal_code = models.CharField(
        ...
        validators=[
            RegexValidator(
                regex=r"^\d{5}(?:[-\s]\d{4})?$", message="Invalid postal code"
            )
        ],
...
class Currency(models.Model):
    ...
    exchange_rate = models.DecimalField(
        max_digits=10,
        decimal_places=4,
        help_text="The exchange rate of the currency.",
        null=True,
    )
    created_at = models.DateTimeField(
        auto_now_add=True,
        help_text="The date and time the currency was created.",
        null=True,
    )
    updated_at = models.DateTimeField(
        auto_now=True, help_text="The date and time the currency was last updated."
    )
    description = models.TextField(
        blank=True, null=True, help_text="Additional information about the currency."
    )
    status = models.CharField(
        max_length=10,
        choices=STATUS_CHOICES,
        default=ACTIVE,
        help_text="Indicates if the currency is active or not.",
    )

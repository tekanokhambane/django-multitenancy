...
from multitenancy.invitations.models import JoinInvitation
...
class BaseTeam(models.Model):
    ...
    member_access = models.CharField(
        max_length=20, choices=MEMBER_ACCESS_CHOICES, verbose_name=_("member access")
    )
    manager_access = models.CharField(
        max_length=20, choices=MANAGER_ACCESS_CHOICES, verbose_name=_("manager access")
    )
    ...
    def can_apply(self, user):
        state = self.state_for(user)
        return (
            self.member_access == BaseTeam.MEMBER_ACCESS_APPLICATION and state is None
        )
    ...
    def acceptances(self):
        return self.memberships.filter(
            state__in=[BaseMembership.STATE_ACCEPTED, BaseMembership.STATE_AUTO_JOINED]
        )
    ...
    def is_owner_or_manager(self, user):
        return self.acceptances.filter(
            role__in=[BaseMembership.ROLE_OWNER, BaseMembership.ROLE_MANAGER], user=user
        ).exists()
    ...
    def add_user(self, user, role, by=None):
        ...
        membership, _ = self.memberships.get_or_create(
            user=user, defaults={"role": role, "state": state}
        )
        signals.added_member.send(sender=self, membership=membership, by=by)
        return membership
...
class Team(BaseTeam):
    ...
    avatar = models.ImageField(
        upload_to=avatar_upload, blank=True, verbose_name=_("avatar")
    )
...
class BaseMembership(models.Model):
    ...
    state = models.CharField(
        max_length=20, choices=STATE_CHOICES, verbose_name=_("state")
    )
    role = models.CharField(
        max_length=20, choices=ROLE_CHOICES, default=ROLE_MEMBER, verbose_name=_("role")
    )

...
class TeamSignupView(SignupView):
    ...
    template_name = "multitenancy/teams/signup.html"
    ...
    def after_signup(self, form):
        if not self.signup_code:
            self.created_user.teams_created.create(name=form.cleaned_data["team"])
...
class TeamCreateView(LoginRequiredMixin, CreateView):
    ...
    form_class = TeamForm
    model = Team
    template_name = "multitenancy/teams/team_form.html"
...
class TeamListView(ListView):
    ...
    template_name = "multitenancy/teams/team_list.html"
...
def team_update(request):
    ...
    return render(
        request, "multitenancy/teams/team_form.html", {"form": form, "team": team}
    )
...
def team_detail(request):
    ...
    return render(
        request,
        "multitenancy/teams/team_detail.html",
        {
            "team": team,
            "state": state,
            "role": role,
...
class TeamManageView(TemplateView):
    ...
    template_name = "multitenancy/teams/team_manage.html"
...
def team_join(request):
    ...
    if (
        team.manager_access == Team.MEMBER_ACCESS_INVITATION
        and state is None
...
def team_leave(request):
    ...
    if team.can_leave(request.user) and request.method == "POST":
        ...
        return redirect("multitenancy_teams:dashboard")
    else:
        return redirect(team.get_absolute_url())
...
class TeamInviteView(FormView):
    ...
    def get_form_success_data(self, form):
        ...
        data = {
            "html": render_to_string(
                "multitenancy/teams/_invite_form.html",
                {"invite_form": self.get_unbound_form(), "team": self.team},
            ...
            data.update(
                {
                    "append-fragments": {
                        fragment_class: render_to_string(
                            "multitenancy/teams/_membership.html",
                            {"membership": membership, "team": self.team},
                            request=self.request,
                        )
                    }
                }
    ...
    def form_valid(self, form):
        ...
        if isinstance(user_or_email, str):
            self.membership = self.team.invite_user(
                self.request.user, user_or_email, role
            )
        else:
            self.membership = self.team.add_user(
                user_or_email, role, by=self.request.user
            )
    ...
    def form_invalid(self, form):
        data = {
            "html": render_to_string(
                "multitenancy/teams/_invite_form.html",
                {"invite_form": form, "team": self.team},
                request=self.request,
            )
        }
        return self.render_to_response(data)
...
def autocomplete_users(request):
    ...
    users = User.objects.exclude(
        pk__in=[x.user.pk for x in team.memberships.exclude(user__isnull=True)]
    )

...
from ..core.forms import CustomerAddTenantForm
...
def customer_dashboard(request):
    ...
    return render(request, 'customer_template/home_content.html', {})

@login_required
def sidebar(request):
    customer = Customers.objects.filter(user_id=request.user)
    return render(request, 'customer_template/sidebar_template.html', {'customer': customer})
...
def customer_view_accounts(request):
   ...
   # user = Customers.objects.get(id=request.user)
    ...
    return render(request, 'customer_template/customer_view_accounts.html', {'tenants':tenants, })
...
def customer_view_domain(request):
    ...
    url = Domain.objects.filter(tenant=user)
    #url = Client.objects.filter()

    return render(request, 'customer_template/customer_view_domains.html', {'url':url})
...
def profile(request):
   # user=TenantUser.objects.get(id=request.user.id)
    ...
    return render(request, 'customer_template/profile.html', {'nbar': 'admin_profile', 'administrator':administrator, 'location':location })
...
def create_tenant(request):
    ...
    return render(request, 'customer_template/add_tenant.html', {'nbar': 'create_tenant', 'form': form})
    ...
    if request.method != "POST":
        ...
        # tenant = None
           ...
           # if not user.is_active:
            ...
            return render(request, 'customer_template/create_tenant.html', {'nbar': 'create_tenant', 'form': form})

...
def admin_home(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1":                  
                return render(request, 'portal/hod_template/home_content.html',
                            {'nbar': 'admin',
                            'title': 'Dashboard!',
                            'tenants': tenants,
                            'customers': customers,
...
def admin_profile(request):
   # user=TenantUser.objects.get(id=request.user.id)
    ...
    if user.is_authenticated:
            if user.user_type == "1":              
                return render(request, 'portal/hod_template/admin_profile.html', {'nbar': 'admin_profile', 'administrator':administrator })
...
def admin_profile_manage(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/admin_profile.html', {'nbar': 'admin_profile'})
...
def edit_profile(request, admin_id):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/admin_profile.html', {'adminhod': adminhod, 'tenants': tenants})
...
def add_staff(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/add_staff.html', {'nbar': 'add_staff', 'form': form})
...
def staff_quick_create(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1": 
                return render(request, 'portal/hod_template/modals/staff_create_form.html', {'nbar': 'add_staff', 'form': form, 'departments': departments})
    ...
    if request.method != "POST":
        ...
        if form.is_valid():
           ...
           # department = form.cleaned_data["department"]
            ...
            form=AddStaffForm(request.POST)
            return render(request, 'portal/hod_template/manage_staff.html', {'nbar': 'add_staff', 'form': form})
...
def manage_staff(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/manage_staff.html', {'nbar': 'manage_staff', 'staff': staff})
...
def manage_customer(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/manage_customer.html', {'nbar': 'manage_customer', 'customers': customers})
...
def edit_staff(request, staff_id):    
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/edit_staff_template.html', {'nbar': 'manage_staff','staff': staff, "form":form},)
    ...
    if request.method != "POST":
        ...
        if form.is_valid():
           ...
           # password = form.cleaned_data["password"]
            ...
            form=EditstaffForm(request.POST)
            staff = Staff.objects.get(admin=staff_id)
            return render(request, 'portal/hod_template/edit_staff_template.html', {'staff':staff, 'form':form, "id":staff_id, "username":username})


class StaffDeleteView(LoginRequiredMixin, DeleteView):
    model = Staff
    template_name = "portal/hod_template/delete_staff.html"
    success_url = reverse_lazy("manage_staff")


class StaffDetailView(LoginRequiredMixin, DetailView):
    model = Staff
    template_name = "portal/hod_template/view_staff.html"
...
def customer_quick_create(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1": 
                return render(request, 'portal/hod_template/modals/customer_create_form.html', {'nbar': 'add_customer', 'form': form})
...
def quick_customer_save(request):
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            return render(request, 'portal/hod_template/manage_customer.html', {'nbar': 'add_customer', 'form': form})
...
class CustomerDeleteView(LoginRequiredMixin, DeleteView):
    model = Customers
    template_name = "portal/hod_template/delete_customer.html"
    success_url = reverse_lazy("manage_customer")

class CustomerDetailView(LoginRequiredMixin, DetailView):
    model = Customers
    template_name = "portal/hod_template/view_customer.html"    
...
def edit_customer(request, customer_id):
   ...
   #form.fields['address_line_1'].initial= location.address.address_line_1
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/edit_customer_template.html', {'nbar': 'manage_customer', "form":form, 'id': customer_id, "customer": customer, },)
...
def edit_customer_save(request):
    if request.method != "POST":
        ...
        # password = request.POST.get("password")
            ...
            form=EditCustomerForm(request.POST)
            customer = Customers.objects.get(admin=customer_id)
            return render(request, 'portal/hod_template/edit_customer_template.html', {'customer':customer, 'form':form, "id":customer_id})
...
def add_tenant(request):
    form = AddTenantForm()
    return render(request, 'portal/hod_template/add_tenant.html', {'nbar': 'add_tenant', 'form': form})
           ...
           # if not user.is_active:
            ...
            if TENANT_SCHEMAS:
                ...
                try:
                    ...
                    return HttpResponseRedirect("admin/tenants/view")
                except:
...
def manage_tenant(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/manage_tenant.html', {'nbar': 'manage_tenant', 'tenant': tenant})
...
def edit_tenant(request, tenant_id):
   ...
   # form.fields['owner_id'].initial=tenant.owner_id
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/edit_tenant_template.html', {'nbar': 'manage_tenant', 'tenant': tenant, 'form':form})
    ...
    if request.method !="POST":
        ...
        if form.is_valid():
           ...
           # owner_id = form.cleaned_data["owner_id"]        
            ...
            form=EditTenantForm(request.POST)
            tenant = get_tenant_model().objects.get(pk=tenant_id)
            return render(request, 'portal/hod_template/edit_tenant_template.html', {'tenant':tenant, 'form':form, "id":tenant_id})
...
def add_tenant_domain(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1": 
                return render(request, 'portal/hod_template/add_domain_template.html', {'tenant': tenant, 'domain': domain, 'nbar': 'add_tenant_domain'},)
...
def manage_domain(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/manage_domain.html', {'nbar': 'manage_domain', 'domain': domain})
...
def edit_domain(request, domain_id):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/edit_domain_template.html', {'nbar': 'manage_domain', 'domain':domain,})
...
class DepartmentListView(ListView, LoginRequiredMixin):
    model = Department
    template_name = 'portal/hod_template/department_list.html'
    #fields = ['name']
...
def create_department_view(request):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/modals/department_create_form.html', {'nbar': 'add_customer', 'form': form})
...
def quick_department_save(request):
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            return render(request, 'portal/hod_template/modals/department_create_form.html', {'nbar': 'department', 'form': form})

@login_required
def address_list(request):
    address = Address.objects.all().exclude(id=1)
    return render(request, 'portal/hod_template/address_list.html', {"address": address})
...
class CompanyDetailsView(TemplateView, LoginRequiredMixin):
   # model = CompanyDetails
    template_name = 'portal/hod_template/company_detail.html'
...
class CompanyUpdateView(UpdateView, LoginRequiredMixin):
   ...
   # fields = ["name", "logo", "website", "phone", "email"]
    ...
    template_name = 'portal/hod_template/companydetails_form.html'
...
def update_company(request, company_id):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/update_company-details.html', {'nbar': 'update_company', "form":form, 'company_id': company_id},)
...
def update_company_address(request, address_id):
    ...
    if user.is_authenticated:
            if user.user_type == "1":
                return render(request, 'portal/hod_template/update_company_address.html', {'nbar': 'update_company', "form":form, 'company_id': address_id},)
